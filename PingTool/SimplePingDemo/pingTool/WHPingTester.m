...
- (instancetype) initWithHost {
    self = [super init];
    if (self) {
        self.simplePing = [[SimplePing alloc] initWithHost];
        self.simplePing.delegate = self;
        self.simplePing.addressStyle = SimplePingAddressStyleAny;
        self.pingItems = [NSMutableArray new];
    }
...
- (void)setHostName:(NSString *)hostName {
    _hostName = hostName;
    self.simplePing.hostName = hostName;
}

- (void)setPingArray:(NSMutableArray *)pingArray {
    _pingArray = pingArray;
    for (int i = 0; i < pingArray.count; i++) {
        self.simplePing.hostName = pingArray[i];
    }
...
- (void)simplePing:(SimplePing *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if([self.pingItems containsObject:item])
        {
            ...
            if(self.delegate!=nil && [self.delegate respondsToSelector:@selector(didPingSucccessWithTime:withHostName:withError:)])
            {
                [self.delegate didPingSucccessWithTime:0 withHostName:self.hostName withError:[NSError errorWithDomain:NSURLErrorDomain code:111 userInfo:nil]];
            }
        }
    });
}
- (void)simplePing:(SimplePing *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error
{
    ...
    if(self.delegate!=nil && [self.delegate respondsToSelector:@selector(didPingSucccessWithTime:withHostName:withError:)])
    {
        [self.delegate didPingSucccessWithTime:0 withHostName:self.hostName withError:error];
    }
}
...
- (void)simplePing:(SimplePing *)pinger didReceivePingResponsePacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber
{
    ...
    if(self.delegate!=nil && [self.delegate respondsToSelector:@selector(didPingSucccessWithTime:withHostName:withError:)])
    {
        [self.delegate didPingSucccessWithTime:delayTime withHostName:self.hostName withError:nil];
    }
}
